...
**如果不做均值归一化的话，那么最后根据梯度下降的得到的$$\theta^{(5)}$$都是0，因为第Eve并没有对电影进行任何评分，前面的第一个式子$$\frac{1}{2}\sum_{(i,j):r(i,j)=1}((\theta^{(j)})^Tx^{(i)}-y^{(i,j)})^2$$并没有关于$$\theta^{(5)}$$的事，所以最后损失函数最小的时候，那就是$$\theta^{(5)}$$每一项最小，全都是0。使用均值归一化，并不会影响到其他的值，还会得到正常的评分，虽然$$\theta^{(5)}$$还是0。**
