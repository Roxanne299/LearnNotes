...
34. 后端生成时间是正确的时区 但是在前端就不一样了，在`pojo`中对应的变量`JsonFormat`注释中加上`,timezone = "Asia/Shanghai"`

35. 前端实现`${}`字符串拼接失效，`  const socketUrl = 'ws://127.0.0.1:8081/websocket/${store.state.user.id}/';`,将引号换成倒引号`就可以用了
